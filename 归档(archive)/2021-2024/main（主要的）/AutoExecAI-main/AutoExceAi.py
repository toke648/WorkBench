...
def is_safe_command(command):
    blacklisted = ["rm -rf", "rm", "del", "dd if=", ":(){ :|: & };:"]
    if blacklisted in command:
        print(f"[WARNING] 检测到危险命令: {command}")
        return False
    else:
        return True
# **主程序**
def main():
    ...
    while True:
        ...
        # **执行终端命令**
        if is_safe_command(parsed["terminal"]) == True:
            feedback = execute_commands(parsed["terminal"])
        else:
            confirm = input("[WARNING] 检测到危险命令，是否继续执行？(yes/no): ").strip().lower()
            if confirm != 'yes':
                print("[INFO] 用户取消执行")
                continue
            else:
