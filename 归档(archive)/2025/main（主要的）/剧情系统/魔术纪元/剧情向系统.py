...
_plot_dialogue_path = path + "\魔术纪元\Dialogue\plot_dialogue_xxx_001.xlsx"
plot_dialogue = pd.read_excel(_plot_dialogue_path) # message_id message text_type user_id user_name
print(_plot_dialogue_path)
print(plot_dialogue)

_plot_path = path + "\魔术纪元\Plot\plot_xxx_001.xlsx"
plot = pd.read_excel(_plot_path)
print(_plot_path)
print(plot)

_plot_dialogue_end_path = path + "\魔术纪元\Dialogue\End\plot_dialogue_xxx_end_.xlsx"
...
_plot_dialogue_message_path = path + "\魔术纪元\Dialogue\Message\plot_dialogue_xxx_001_message_.xlsx"
...
_plot_option_path = path + "\魔术纪元\Dialogue\Option\option_dialogue_xxx_option_001.xlsx"
...
class Player:
    ...
    def run(self):
        ...
        while True:
            row = plot_dialogue[plot_dialogue['dialogue_id'] == current_dialogue_id].iloc[0]
            dia_type = row['dialogue_type']
            next_id = row['next_dialogue_id']

            print(f"\n当前 DIALOGUE: {current_dialogue_id}, 类型: {dia_type}")

            # NORMAL
            if dia_type == 'normal':
                related = plot_dialogue_message[plot_dialogue_message['dialogue_id'] == current_dialogue_id]
                for _, msg in related.iterrows():
                    print(f"{msg['order']}. {msg['user_name']}：{msg['text']}")
                current_history.append(current_dialogue_id)
                current_dialogue_id = next_id  # 自动转移
                continue

            # OPTION
            elif dia_type == 'option':
                related = plot_dialogue_option[plot_dialogue_option['dialogue_id'] == current_dialogue_id]
                for _, opt in related.iterrows():
                    print(f"{opt['order']}: {opt['option_text']}")

                choice = int(input("请选择："))
                selected = related[related['order'] == choice].iloc[0]
                print(f"你选择了：{selected['option_text']}")

                current_history.append(current_dialogue_id)
                current_dialogue_id = selected['next_dialogue_id']
                continue

            # END
            elif dia_type == 'end':
                print("=== 结局 ===")
                related_end = plot_dialogue_end[plot_dialogue_end['currect_history'] == str(current_history)]
