# policy_assistant_fixed.py
""" utf-8 encoding 智能政策查询助手 - 修复网络问题版本 """
import os
import json
import requests
import gradio as gr
...
class FixedPolicyKnowledgeBase:
    """修复网络问题的政策知识库管理类"""
    ...
    def get_embedding(self, text):
        ...
        try:
            # 使用GLM的嵌入功能（如果没有，使用文本相似度计算作为备选）
            response = self.openai_client.embeddings.create(
                model="embedding-2",  # 或者使用可用的嵌入模型
                input=text
            )
            return response.data[0].embedding
        except:
            # 如果嵌入API不可用，使用简单的TF-IDF风格备选方案
...
# 创建全局实例
knowledge_base = FixedPolicyKnowledgeBase()
...
if __name__ == "__main__":
    ...
    print("✅ 使用OpenAI API进行文本嵌入，无需下载本地模型")
