...
class RealTimeFaceRecognition:
    def __init__(self, camera_id=0, display_size=(800, 600)):
        ...
        # --- 新增：识别结果缓存与窗口内输入状态 ---
        self.last_recognitions = []   # 缓存上一帧的识别结果，保证画面持续显示身份
        self.typing_name_mode = False # 是否处于“注册姓名输入模式”
        self.name_buffer = ""         # 当前输入的姓名缓冲
        # --- 结束新增 ---
        ...
        while True:
            ...
            if current_time - last_recognition_time > recognition_interval:
                if detections:
                    ...
                    # 新增：识别结果的实时输出（与检测对应）
                    for idx, rec in enumerate(recognitions, start=1):
                        name, similarity, _ = rec
                        x1, y1, x2, y2, conf = detections[idx - 1]
                        print(f"  -> 人脸#{idx} 识别: {name} (sim={similarity:.2f}, conf={conf:.2f}) box=({x1},{y1},{x2},{y2})")
            ...
            elif key == ord('r'):  # 注册新人脸
                name = input("请输入姓名: ").strip()
                if name:
                    self.register_new_face(display_frame, name)
