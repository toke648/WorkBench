...
def extract_formatted_chat_history():
    ...
    for i, conversation in enumerate(conversations):
        ...
        #     # print(key, value.get('parent'))
        #     print(json.dumps(value.get('message'), ensure_ascii=False, indent=2))
        #     # chat_session['messages'] = value['message']['']

        print(json.dumps(conversation['mapping'])) # 输出mapping字典 ensure_ascii=False 用于显示中文字符 indent=2 的作用是格式化输出，使嵌套结构更清晰可见
        # print(conversation['mapping'].get('text_content', '无内容'))
        
        
        # for key, value in conversation['mapping'].items():
        #     # print(json.dumps(value.get('message'), ensure_ascii=False, indent=2))
        #     # print(key, value.get('content', '无内容'))
        #     print(value['content']['parts'][0]['text'])
